...
开发模拟器启动后会在 `8090` 端口等待 App 配网，并提供附近 Wi-Fi 列表。App 创建一次性绑定令牌，用户选择 Wi-Fi 并填写密码后交给主机；主机向后端认领并上线后，App 再通过 FastAPI 设置模型服务和 API Key。FastAPI 只实时转发配置，Key 最终保存在主机本地。真实硬件上，`EXPO_PUBLIC_HOST_SETUP_URL` 应指向主机热点内的固定地址和配网端口，例如 `http://192.168.4.1:8090`。
